...
URL = 'https://p2.sshreach.me/029GCWpxx8KYDPv2N87XDHQtlhGzJQJG'
PORTS_ID = '13804'
UNIX_USERNAME = 'ClQRGM6eNjL5lwQxLndNdCfNmF0pPP'
HOST_UUID = '3f8d84a0-5b66-4ac5-9b1f-50a88f38d54f'
USER_ID = '20169'
...
def startSSH(data, private_key):
    try:
        if os.name == 'posix':       # if os is linux or mac
            ...
            if pid:
                ...
                if dropbear == True:
                    if StrictHostKeyChecking:
                        ...
                        process = 'dbclient -N -i {0} -R {1}:{5}:{3} {4}@{2} &'.format(private_key,
                            data['iport'], data['forwarding_server'], FORWARD_PORT, UNIX_USERNAME, ADDRESS)
                    else:
                        # The line below should not be changed. If the dbclient command is changed then the corresponding line in get_pid() must
                        # also be changed otherwise the script might not recognize that dbclient connection is active and will try to open it
                        # again and again resulting in hundreds of inactive dbclient processes.
                        process = 'dbclient -y -N -i {0} -R {1}:{5}:{3} {4}@{2} &'.format(private_key,
                            data['iport'], data['forwarding_server'], FORWARD_PORT, UNIX_USERNAME, ADDRESS)
                else:
                    if StrictHostKeyChecking:
                        ...
                        process = 'ssh {6} -N -i {0} -R {1}:{5}:{3} {4}@{2} &'.format(private_key,
                            data['iport'], data['forwarding_server'], FORWARD_PORT, UNIX_USERNAME, ADDRESS, sshrsaoption)
                    else:
                        # The line below should not be changed. If the ssh command is changed then the corresponding line in get_pid() must
                        # also be changed otherwise the script might not recognize that ssh connection is active and will try to open it
                        # again and again resulting in hundreds of inactive ssh processes.
                        process = 'ssh -o StrictHostKeyChecking=no {6} -o LogLevel=ERROR -N -i {0} -R {1}:{5}:{3} {4}@{2} &'.format(private_key,
                            data['iport'], data['forwarding_server'], FORWARD_PORT, UNIX_USERNAME, ADDRESS, sshrsaoption)
            ...
            process = '{6} -N -batch -i {0} -R {1}:{5}:{3} {4}@{2} &'.format(private_key,
                data['iport'], data['forwarding_server'], FORWARD_PORT, UNIX_USERNAME, ADDRESS,
        ...
        try:
            ...
            if dropbear == True:
                if StrictHostKeyChecking:
                    ...
                    # also be changed otherwise the script might not recognize that dbclient connection is active and will try to open it
                    # again and again resulting in hundreds of inactive dbclient processes.
                    checkline = 'dbclient -N -i {0} -R {1}:{5}:{3} {4}@{2}'.format(private_key, 
                        iport, forwarding_server, FORWARD_PORT, UNIX_USERNAME, ADDRESS)
                else:
                    # The line below should not be changed. If the dbclient command is changed then the corresponding line in startSSH() must
                    # also be changed otherwise the script might not recognize that dbclient connection is active and will try to open it
                    # again and again resulting in hundreds of inactive dbclient processes.
                    checkline = 'dbclient -y -N -i {0} -R {1}:{5}:{3} {4}@{2}'.format(private_key, 
                        iport, forwarding_server, FORWARD_PORT, UNIX_USERNAME, ADDRESS)
            else:
                if StrictHostKeyChecking:
                    ...
                    # also be changed otherwise the script might not recognize that ssh connection is active and will try to open it
                    # again and again resulting in hundreds of inactive ssh processes.
                    checkline = 'ssh {6} -N -i {0} -R {1}:{5}:{3} {4}@{2}'.format(private_key, 
                        iport, forwarding_server, FORWARD_PORT, UNIX_USERNAME, ADDRESS, sshrsaoption)
                else:
                    # The line below should not be changed. If the ssh command is changed then the corresponding line in startSSH() must
                    # also be changed otherwise the script might not recognize that ssh connection is active and will try to open it
                    # again and again resulting in hundreds of inactive ssh processes.
                    checkline = 'ssh -o StrictHostKeyChecking=no {6} -o LogLevel=ERROR -N -i {0} -R {1}:{5}:{3} {4}@{2}'.format(private_key, 
                        iport, forwarding_server, FORWARD_PORT, UNIX_USERNAME, ADDRESS, sshrsaoption)
